...
export const ScanSwapView: React.FC<ScanSwapViewProps> = ({ onBack, user, balances, onRefresh, setBalances }) => {
  ...
  if (step === 'scan') {
    return (
      <div className="h-full flex flex-col bg-black relative pb-20">
        <Button variant="ghost" className="absolute top-4 left-4 z-20 text-white" onClick={onBack}>
        ...
        <div className="flex-1 relative overflow-hidden flex items-center justify-center">
           ...
           <div className="w-64 h-64 border-2 border-white/50 rounded-3xl relative z-10 flex items-center justify-center">
             <div className="w-60 h-0.5 bg-red-500 absolute top-1/2 -translate-y-1/2 shadow-[0_0_10px_red] animate-[ping_1.5s_cubic-bezier(0,0,0.2,1)_infinite]" />
             <QrCode className="w-12 h-12 text-white/20" />
           </div>
           
           <div className="absolute bottom-10 w-full px-6 space-y-4 z-20">
                <div className="bg-surface/80 backdrop-blur-md p-4 rounded-2xl border border-white/10">
                    <p className="text-center text-white text-sm mb-3 font-medium">Developer Simulation Mode</p>
                    <div className="flex gap-3">
                        <button className="flex-1 py-3 bg-white text-black rounded-xl text-xs font-bold active:scale-95 transition-transform" onClick={() => processInput("IDRX:MERCHANT01")}>
                            Simulate QRIS
                        </button>
                        <button className="flex-1 py-3 bg-neutral-800 text-white border border-white/20 rounded-xl text-xs font-bold active:scale-95 transition-transform" onClick={() => processInput("mvines9ez1GcuGeR4xpE6Ete25N2del65")}>
                            Simulate SOL Addr
                        </button>
                    </div>
                </div>
      ...
      return (
        <div className="pb-24 pt-4">
            ...
            <div className="mt-8 text-center flex flex-col items-center">
                ...
                <h2 className="text-xl font-bold text-white mb-6 bg-surface px-4 py-2 rounded-lg border border-border">{txDetails.to}</h2>
                
                <div className="w-full h-px bg-border mb-6"></div>

                <p className="text-5xl font-mono font-bold mb-2 tracking-tighter">
                    {txDetails.amount.toLocaleString()} 
                </p>
                <span className="text-lg text-secondary font-medium mb-12 block">{isSol ? "SOL" : "IDRX"}</span>

                <Button fullWidth onClick={isSol ? handleSolTransfer : handleIdrxPayment} isLoading={loading}>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-center space-y-6 animate-in zoom-in duration-300">
      ...
      <Button variant="outline" onClick={onBack} className="mt-8">Return Home</Button>
    </div>
  );
};